...
void process_send_packet(unsigned char* pkt, int pkt_len, int id,struct nfq_q_handle* qh){
...
static int cb(struct nfq_q_handle* qh, struct nfgenmsg* nfmsg, struct nfq_data* nfa, void *data){
    ...
    if(!is_failure){
		...
		//*********************************************************************
			if(destinationAddress==0 && is_send == 1){
				destinationAddress = iphdrp->daddr;
			}
...
int main(int argc, char** argv){
	...
	//**********create the failure detection thread************
	pthread_t tid;
